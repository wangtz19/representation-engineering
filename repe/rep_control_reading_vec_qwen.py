...
class WrappedReadingVecModel(torch.nn.Module):
    ...
    def wrap(self, layer_id, block_name):
        ...
        if self.is_wrapped(self.model.transformer.h[layer_id]):
            block = getattr(self.model.transformer.h[layer_id].block, block_name)
            if not self.is_wrapped(block):
                setattr(self.model.transformer.h[layer_id].block, block_name, WrappedBlock(block))
        else:
            block = getattr(self.model.transformer.h[layer_id], block_name)
            if not self.is_wrapped(block):
                setattr(self.model.transformer.h[layer_id], block_name, WrappedBlock(block))
    ...
    def unwrap(self):
        for l, layer in enumerate(self.model.transformer.h):
            ...
            for block_name in BLOCK_NAMES:
                if self.is_wrapped(getattr(self.model.transformer.h[l], block_name)):
                    setattr(self.model.transformer.h[l],
                            block_name,
                            getattr(self.model.transformer.h[l], block_name).block)
